  ...
  <div class="min-h-screen bg-gradient-to-br from-[#FDFCFD] via-[#FFF5F9] to-[#EAF6FF]">
    ...
    <section class="relative overflow-hidden bg-gradient-to-br from-[#FF4D94] via-[#7C4DFF] to-[#FF4D94] py-20 lg:py-32">
      <div class="absolute inset-0 opacity-20">
        ...
        <div class="absolute bottom-[-10%] right-[-10%] w-[50%] h-[50%] bg-white rounded-full blur-[100px] animate-pulse"></div>
      </div>
      ...
      <div class="container mx-auto px-4 relative z-10 text-center text-white">
        ...
        <div class="flex flex-wrap gap-4 justify-center mb-16">
          ...
          <a href="https://www.familybank.chat" target="_blank" class="group inline-flex items-center gap-3 px-8 py-4 bg-white/10 backdrop-blur-md text-white rounded-2xl font-black text-lg border-2 border-white/30 hover:bg-white/20 transition-all">
            <Zap class="w-5 h-5" />
            立即体验
            <ArrowRight class="w-5 h-5 group-hover:translate-x-1 transition-transform" />
          </a>
    ...
    <div class="container mx-auto px-4 py-16">
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div class="lg:col-span-8 space-y-12">
          ...
          <section v-if="homeData?.featuredPosts?.length">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
              <article v-for="(post, index) in homeData.featuredPosts" :key="post.id" 
                class="group bg-white rounded-3xl border border-gray-100 hover:border-[#FF4D94]/30 hover:shadow-2xl transition-all duration-300 overflow-hidden relative">
                <!-- Rank Badge -->
                <div class="absolute top-4 left-4 z-10 w-10 h-10 rounded-xl flex items-center justify-center text-sm font-black text-white shadow-lg"
                ...
                <NuxtLink :to="`/blog/${post.slug}`" class="block p-6">
                  ...
                  <div class="flex items-center gap-3 mb-4">
                    <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-[#FF4D94] to-[#7C4DFF] overflow-hidden border-2 border-gray-100 shadow-sm">
                      <img v-if="post.profiles?.avatar_url" :src="post.profiles.avatar_url" class="w-full h-full object-cover" />
                      <User v-else class="w-5 h-5 m-2.5 text-white" />
                    </div>
                    <div class="flex-1 min-w-0">
                      <div class="text-sm font-black text-gray-900 truncate">{{ formatAuthorName(post.profiles) }}</div>
                  ...
                  <p v-if="post.excerpt" class="text-sm text-gray-600 line-clamp-2 mb-4 leading-relaxed font-medium">
                    {{ post.excerpt }}
                  </p>
          ...
          <section v-if="homeData?.categories?.length">
            <div class="flex items-center justify-between mb-8">
              ...
              <NuxtLink to="/categories" class="text-[#FF4D94] font-black text-sm uppercase flex items-center gap-2 hover:text-[#7C4DFF] transition-colors group">
            ...
            <div class="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
              ...
              <div class="divide-y divide-gray-100">
                <article v-for="cat in homeData.categories" :key="cat.id" class="hover:bg-gradient-to-r hover:from-[#FF4D94]/5 hover:to-[#7C4DFF]/5 transition-all group">
                  <div class="grid grid-cols-1 md:grid-cols-12 gap-4 px-6 py-5">
                    <NuxtLink :to="`/categories/${cat.slug}`" class="col-span-12 md:col-span-5 flex items-center gap-4">
                      ...
                      <div class="flex-1 min-w-0">
                        <h3 class="font-black text-base sm:text-lg text-gray-900 mb-1 group-hover:text-[#FF4D94] transition-colors">{{ cat.name }}</h3>
                        <p v-if="cat.description" class="text-xs sm:text-sm text-gray-600 font-medium line-clamp-1 break-words truncate">{{ cat.description }}</p>
                      </div>
                    </NuxtLink>
                      ...
                      <div v-if="cat.latestPost" class="flex-1 min-w-0">
                        <NuxtLink :to="`/blog/${cat.latestPost.slug}`" class="block group/post">
                          <div class="flex items-center gap-2 mb-1">
                            ...
                            <span class="text-xs sm:text-sm font-black text-gray-900 group-hover/post:text-[#FF4D94] transition-colors line-clamp-1">{{ cat.latestPost.title }}</span>
                          </div>
                          <div class="flex flex-wrap items-center gap-2 text-[10px] sm:text-xs text-gray-500 font-bold">
                            ...
                            <span>•</span>
                            <span class="flex items-center gap-1"><MessageCircle class="w-3 h-3" /> {{ cat.latestPost.commentCount }}</span>
                            <span>•</span>
                            <span class="flex items-center gap-1"><Eye class="w-3 h-3" /> {{ cat.latestPost.view_count }}</span>
                            <span>•</span>
                            <span class="font-medium text-gray-400 capitalize">{{ formatDate(cat.latestPost.published_at) }}</span>
                          </div>
                        </NuxtLink>
                      </div>
                      <div v-else class="text-xs sm:text-sm text-gray-400 font-medium uppercase tracking-widest">暂无动态</div>
                    </div>
                  </div>
                </article>
              </div>
          ...
          <section v-if="homeData?.recentComments?.length">
            <div class="flex items-center justify-between mb-8">
              ...
              <NuxtLink to="/blog" class="text-[#FF4D94] font-black text-sm uppercase flex items-center gap-2 hover:text-[#7C4DFF] transition-colors group">
                查看更多回复 <ArrowRight class="w-4 h-4 group-hover:translate-x-1 transition-transform" />
              </NuxtLink>
            </div>
        ...
        <aside class="lg:col-span-4 space-y-8">
          ...
          <section v-if="homeData?.hotPosts?.length" class="bg-white rounded-3xl border border-gray-100 p-6 shadow-sm">
            ...
            <div class="space-y-4">
              <NuxtLink v-for="(post, idx) in homeData.hotPosts" :key="post.id" :to="`/blog/${post.slug}`" 
                ...
                <div class="w-8 h-8 rounded-lg flex items-center justify-center text-xs font-black shadow-sm flex-shrink-0"
                ...
                <div class="flex-1 min-w-0">
                  <h4 class="text-sm font-black text-gray-900 group-hover:text-[#FF4D94] line-clamp-2 mb-2 transition-colors leading-snug">{{ post.title }}</h4>
                  <div class="flex items-center gap-2 text-[10px] font-bold text-gray-400 uppercase tracking-widest">
                    <span class="flex items-center gap-1"><Eye class="w-3 h-3" /> {{ post.view_count }} 阅读</span>
                  </div>
                </div>
              </NuxtLink>
            </div>
          ...
          <!-- Download Card -->
          <div class="bg-gradient-to-br from-[#FF4D94] to-[#7C4DFF] rounded-3xl p-8 text-white shadow-2xl relative overflow-hidden group">
            <div class="absolute top-0 right-0 w-32 h-32 bg-white/10 rounded-full -mr-16 -mt-16 blur-xl group-hover:scale-110 transition-transform"></div>
            <div class="relative z-10">
              <div class="w-12 h-12 rounded-2xl bg-white/20 backdrop-blur-md flex items-center justify-center mb-6 shadow-inner">
                <Download class="w-6 h-6" />
              </div>
              <h3 class="font-black text-xl mb-3 uppercase tracking-tight">下载应用</h3>
              <p class="text-sm text-white/90 mb-8 font-medium leading-relaxed">元气银行家庭积分管理系统，培养孩子好习惯。</p>
              <div class="space-y-3">
                <a href="/download/family-bank.apk" download class="flex items-center justify-center gap-2 px-6 py-3.5 bg-white text-[#FF4D94] rounded-2xl font-black text-sm hover:scale-105 active:scale-95 transition-all shadow-xl">
                  <Smartphone class="w-4 h-4" /> 安卓版下载
                </a>
                <a href="https://www.familybank.chat" target="_blank" class="flex items-center justify-center gap-2 px-6 py-3.5 bg-white/10 border border-white/30 rounded-2xl font-black text-sm hover:bg-white/20 transition-all">
          ...
          <section v-if="homeData?.activeUsers?.length" class="bg-white rounded-3xl border border-gray-100 p-6 shadow-sm">
            <div class="flex items-center gap-3 mb-6">
              <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-yellow-400 to-orange-500 flex items-center justify-center shadow-md">
            ...
            <div class="space-y-3">
              <div v-for="(u, idx) in homeData.activeUsers" :key="u.id" class="flex items-center gap-3 p-3 rounded-2xl hover:bg-gradient-to-r hover:from-[#FF4D94]/5 hover:to-[#7C4DFF]/5 transition-all group">
                ...
                <div class="flex-1 min-w-0">
                  <div class="text-sm font-black text-gray-900 truncate group-hover:text-[#FF4D94] transition-colors lowercase">
          ...
          <!-- Popular Tags -->
          <section v-if="homeData?.tags?.length" class="bg-white rounded-3xl border border-gray-100 p-6 shadow-sm">
            <div class="flex items-center justify-between mb-6">
              <div class="flex items-center gap-3">
                <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-purple-400 to-indigo-500 flex items-center justify-center shadow-md">
                  <Target class="w-5 h-5 text-white" />
                </div>
                <h3 class="font-black text-gray-900 text-lg">热门标签</h3>
              </div>
              <NuxtLink to="/tags" class="text-xs font-black text-[#FF4D94] hover:text-[#7C4DFF] transition-colors uppercase">全部</NuxtLink>
            </div>
            <div class="flex flex-wrap gap-2.5">
              <NuxtLink v-for="tag in homeData.tags" :key="tag.id" :to="`/tags/${tag.slug}`"
                class="px-4 py-2 bg-gradient-to-r from-[#FF4D94]/10 to-[#7C4DFF]/10 hover:from-[#FF4D94] hover:to-[#7C4DFF] rounded-2xl border border-[#FF4D94]/20 hover:border-[#FF4D94] transition-all text-sm font-black text-gray-700 hover:text-white">
                {{ tag.name }}
              </NuxtLink>
            </div>
          </section>
