-- 步骤 3：为现有的孤立用户创建 profile 和 family（修复版）
-- 这个版本会检查 role 约束并使用正确的值
...
DECLARE
  ...
  v_user_name TEXT;
  v_count INT := 0;
  v_role TEXT := 'admin';  -- 尝试使用 'admin' 而不是 'parent'
BEGIN
  ...
  LOOP
    ...
    BEGIN
      ...
      RAISE NOTICE '  -> 创建 profile: % (Role: %)', v_user_name, v_role;
      
    EXCEPTION
      WHEN OTHERS THEN
        RAISE WARNING '  -> 错误: % (SQLSTATE: %)', SQLERRM, SQLSTATE;
        RAISE NOTICE '  -> 尝试使用其他 role 值...';
        
        -- 如果 admin 失败，尝试其他可能的值
        BEGIN
          -- 先删除可能创建的家庭
          ...
          RAISE NOTICE '  -> 使用 author 角色创建成功';
          
        EXCEPTION
          WHEN OTHERS THEN
            RAISE WARNING '  -> 仍然失败: %', SQLERRM;
        END;
    END;
  END LOOP;
